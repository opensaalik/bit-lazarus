...
export function createEnsLocatorAdapterFromEnv(environment = process.env) {
  ...
  if (adapterMode === "viem" || adapterMode === "production") {
    if (!rpcUrl) {
      throw new Error("ENS_RPC_URL or ETH_RPC_URL is required when ENS_ADAPTER=viem");
    }

    if (!privateKey) {
      throw new Error("ENS_PRIVATE_KEY or PRIVATE_KEY is required when ENS_ADAPTER=viem");
    }
  }
